...
features:
  - id: F01
    behavior: "Modelos de datos del post-pago: DownloadGrant / Raffle / RaffleEntry (+ enum RaffleStatus) con onDelete/índices/unique, aplicado con db push, y seed del Raffle ACTIVO vía script"
    state: not_started
...
2. Agregar al schema los modelos `Raffle`, `RaffleEntry`, `DownloadGrant` y el enum `RaffleStatus`, más
   las back-relations en `Order` / `OrderItem` / `Book` (que crea F01-roadmap). Cambio **aditivo**
   aplicado con `db push` (`npm run db:push`) — sin migraciones versionadas. (F01)
...
- **I4**: Todas las FKs nuevas con `onDelete` explícito (`Restrict` sobre modelos auditables) y `@@index`
  en cada FK queriable; cambio **aditivo** aplicado con `db push` (sin migraciones versionadas). Invocar `schema-guardian` antes de tocar el
...
- `schema-guardian` — **antes** de agregar `Raffle`/`RaffleEntry`/`DownloadGrant` + enum + back-relations:
  naming, `onDelete`, `@@index`, `@@unique`, clasificación aditiva/destructiva del `db push`.
...
- [2026-07-16 00:00] [planner-grill] (domain-planner) **SUPERSEDED por el pivote a SaaS multi-tenant** (`tasks/26-07-16-saas-roadmap.md`, ADR-0005). Los efectos post-pago siguen existiendo como Fase F02 del roadmap nuevo, ahora scopeados por tenant (`DownloadGrant`/`Raffle`/`RaffleEntry` con `tenantId`; el `Raffle` ACTIVO se busca dentro de la Tienda de la orden, no globalmente). El contrato del hook post-pago (`({tx, orderId}) => Promise<void>`, una vez, en la misma transacción, solo en pendiente→pagado) sigue válido y se reusa. Este plan no se implementa tal cual: la F02 nueva parirá su propio task file.
